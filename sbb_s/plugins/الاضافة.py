...
@sbb_s.ar_cmd(pattern="انضمام ([\s\S]*)")
async def lol(event):
    a = event.text
    bol = a[5:]
    ...
    try:
        await sbb_s(functions.channels.JoinChannelRequest(bol))
        await event.edit("**- تم الانضمام بنجاح  ✓**")
    except Exception as e:
        await event.edit(str(e))


@sbb_s.ar_cmd(pattern="اضافه ([\s\S]*)")
async def _(event):
...
@sbb_s.ar_cmd(pattern="ضيف ([\s\S]*)", groups_only=True)
async def get_users(event):
    legen_ = event.text[10:]
    sbb_s_chat = legen_.lower
    restricted = ["@Redthon_support", "@Redthon_support"]
    sbb_s = await edit_or_reply(event, f"**جارِ اضأفه الاعضاء من  ** {legen_}")
    if sbb_s_chat in restricted:
        return await sbb_s.edit(
            event, "**- لا يمكنك اخذ الاعضاء من مجموعه السورس العب بعيد ابني  :)**"
        )
    sender = await event.get_sender()
    me = await event.client.get_me()
    if not sender.id == me.id:
        await sbb_s.edit("**▾∮ تتم العملية انتظر قليلا ...**")
    else:
        await sbb_s.edit("**▾∮ تتم العملية انتظر قليلا ...**")
    if event.is_private:
        return await sbb_s.edit("- لا يمكنك اضافه الاعضاء هنا")
    s = 0
    f = 0
    error = "None"
    await sbb_s.edit(
    ...
    async for user in event.client.iter_participants(event.pattern_match.group(1)):
        try:
            ...
            tol = f"@{user.username}"
            lol = tol.split("`")
            await sbb_s(InviteToChannelRequest(channel=event.chat_id, users=lol))
            s = s + 1
            await sbb_s.edit(
                f"**▾∮تتم الأضافة **\n\n• اضيف `{s}` \n•  خطأ بأضافة `{f}` \n\n**× اخر خطأ:** `{error}`"
            )
...
@sbb_s.ar_cmd(pattern="تجميع الاعضاء$")
...
@sbb_s.ar_cmd(pattern="اضف الاعضاء$")
