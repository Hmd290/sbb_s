...
from sbb_s.Config import Config
...
async def promote(event):
    ...
    if not user:
        return
    sbb_sevent = await eor(event, "⌔∮ جارِ رفع المستخدم في الدردشة")
    try:
        await event.client(EditAdminRequest(event.chat_id, user.id, new_rights, rank))
    except BadRequestError:
        return await sbb_sevent.edit(NO_PERM)
...
async def demote(event):
    ...
    if not user:
        return
    sbb_sevent = await eor(event, "⌔∮ جاري تنزيل المستخدم من رتبة الاشراف")
    newrights = ChatAdminRights(
        add_admins=None,
        invite_users=None,
        change_info=None,
...
async def _ban_person(event):
    ...
    sbb_sevent = await eor(event, "**⌔∮ تم حظر هذا المستخدم بنجاح**")
    try:
...
async def nothanos(event):
    ...
    if not user:
        return
    sbb_sevent = await eor(event, "⌔∮ جار الغاء حظر المستخدم")
    try:
        ...
        await sbb_sevent.edit("⌔∮ لقد حدث خطأ اثناء محاولة الغاء الحظر")
    except Exception as e:
        await sbb_sevent.edit(f"**خطأ :**\n`{e}`")


@sbb_s.ar_cmd(incoming=True)
...
@sbb_s.ar_cmd(pattern="كتم(?:\s|$)([\s\S]*)")
...
@sbb_s.ar_cmd(pattern="الغاء كتم(?:\s|$)([\s\S]*)")
...
@sbb_s.ar_cmd(pattern="تثبيت( بالاشعار|$)")
async def pin(event):
...
@sbb_s.ar_cmd(pattern="الغاء تثبيت( الكل|$)")
async def pin(event):
