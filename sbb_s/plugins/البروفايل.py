...
@sbb_s.ar_cmd(pattern="تغيير بايو ([\s\S]*)")
async def _(event):
...
@sbb_s.ar_cmd(pattern="تغيير اسم ([\s\S]*)")
async def _(event):
...
@sbb_s.ar_cmd(pattern="تغيير صورة$")
async def _(event):
    reply_message = await event.get_reply_message()
    sbb_sevent = await edit_or_reply(event, "يتم تحميل الصورة لحسابي ...")
    ...
    try:
        ...
        await sbb_sevent.edit(str(e))
    else:
        if photo:
            await sbb_sevent.edit("**⌔∮ الان يتم التحميل الى تيليكرام ...**")
            if photo.endswith((".mp4", ".MP4")):
                size = os.stat(photo).st_size
                if size > 2097152:
                    await sbb_sevent.edit("⌔∮ الحجم يجب ان يكون اقل من 2 ميغا")
                    os.remove(photo)
                    return
                sbb_spic = None
                sbb_svideo = await event.client.upload_file(photo)
            else:
                sbb_spic = await event.client.upload_file(photo)
                sbb_svideo = None
            try:
                await event.client(
                    functions.photos.UploadProfilePhotoRequest(
                        file=sbb_spic, video=sbb_svideo, video_start_ts=0.01
                    )
                )
            except Exception as e:
                await sbb_sevent.edit(f"**خطأ:**\n{e}")
            else:
                await edit_or_reply(sbb_sevent, "**❃ تم بنجاح تغيير صورة الحساب**")
    try:
        os.remove(photo)
    except Exception as e:
        LOGS.info(str(e))
...
@sbb_s.ar_cmd(pattern="تغيير معرف ([\s\S]*)")
...
async def count(event):
    ...
    b = 0
    result = ""
    sbb_sevent = await edit_or_reply(event, "**⌔∮ يتم جمع معلومات حسابك**")
    ...
    await sbb_sevent.edit(result)


@sbb_s.ar_cmd(pattern="ازاله الصورة ?([\s\S]*)")
