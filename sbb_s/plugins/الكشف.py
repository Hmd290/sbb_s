...
async def fetch_info(replied_user, event):
    ...
    rozrtba = (
        ".「  مطـور السورس  」."
        if user_id == 5411901826 or user_id == 5545368889 or user_id == 5411901826
        else (".「  العضـو  」.")
    )
    rozrtba = (
        ".「 مـالك الحساب  」."
        if user_id == (await event.client.get_me()).id
        and user_id != 5411901826
        and user_id != 5545368889
        and user_id != 5411901826
        else rozrtba
    )
    caption = " \n"
...
@sbb_s.ar_cmd(pattern="كشف(?:\s|$)([\s\S]*)")
async def _(event):
...
@sbb_s.ar_cmd(pattern="ايدي(?:\s|$)([\s\S]*)")
async def who(event):
...
@sbb_s.ar_cmd(pattern="رابط الحساب(?:\s|$)([\s\S]*)")
