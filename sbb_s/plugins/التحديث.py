...
from sbb_s import HEROKU_APP, UPSTREAM_REPO_URL, sbb_s
...
@sbb_s.ar_cmd(pattern="تحديث(| الان)?$")
async def upstream(event):
    ...
    if ENV and (HEROKU_API_KEY is None or HEROKU_APP_NAME is None):
        return await edit_or_reply(
            event, "⌔∮ يجب عليك وضع الفارات المطلوبة لتحديث ردثون"
        )
    try:
        txt = (
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "\n⌔∮ عزيز المستخدم انت تستخدم اخر اصدار من ردثون 🫂♥"
        )
        return repo.__del__()
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"ارسل `{cmdhd}تحديث الان` لتحديث سورس ردثون"
        )
    ...
    if conf == "الان":
        await event.edit("⌔∮ جارِ تحديث ردثون يرجى الأنتظار قليلا")
        await update_bot(event, repo, ups_rem, ac_br)
    return


@sbb_s.ar_cmd(pattern="تحديث التنصيب$",)
async def upstream(event):
    if ENV:
    ...
    off_repo = "https://github.com/Hmd290/RedThon"
    os.chdir("/app")
    try:
        txt = (
            "**⌔∮ عذرا لم يتم اكمال التحديث بسبب بعض الاخطاء "
