#    جميع الحقوق محفوظة كتابة وتعديل  :   @E2EEE7
...
async def get_full_user(event):
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("▾∮ هنالك خطأ يرجى تبليغنا @redthon", str(err))
...
@sbb_s.ar_cmd(pattern="ارفع ?(.*)")
async def gben(sbb_s):
    dc = razan = sbb_s
    i = 0
    await dc.get_sender()
    me = await sbb_s.client.get_me()
    await razan.edit("▾∮ يتم رفع المستخدم في جميع المجموعات")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await sbb_s.get_chat()
    if sbb_s.is_private:
        user = sbb_s.chat
        rank = sbb_s.pattern_match.group(1)
    else:
        sbb_s.chat.title
    try:
        user, rank = await get_full_user(sbb_s)
    except:
        pass
    if me == user:
    ...
    if user:
        telchanel = [
            d.entity.id
            for d in await sbb_s.client.get_dialogs()
        ...
        for x in telchanel:
            try:
                await sbb_s.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
