...
from ..helpers.utils import _sbb_stools, reply_id
...
async def catbot(event):
    ...
    output = await _sbb_stools.media_to_pic(event, replied)
    if output[1] is None:
    ...
    sbb_s = f"https://telegra.ph{response[0]}"
    sbb_s = await trash(sbb_s)
    os.remove(download_location)
    await output[0].delete()
    await event.client.send_file(event.chat_id, sbb_s, reply_to=rozid)
