...
@sbb_s.ar_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**❃ البوت الخاص بك هو** \n {TG_BOT_USERNAME}")


@sbb_s.on(events.NewMessage(outgoing=False, pattern="/roz"))
async def _(event):
...
async def Logo(event):
    ...
    if not text:
        ...
        return
    fnt = await get_font_file(event.client, "@sbb_sfonts")
    ...
    await bot.send_file(
        event.chat_id, file_name, caption=f"⪼ تم صنعه بواسطه : {mention} [@redthon]"
    )
    await evxnt.delete()
    try:
        os.remove(file_name)
