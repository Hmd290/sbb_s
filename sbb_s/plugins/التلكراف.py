...
@sbb_s.ar_cmd(pattern="(ت(ل)?ك(راف)?) ?(m|t|ميديا|نص)(?:\s|$)([\s\S]*)")
async def _(event):
    sbb_sevent = await edit_or_reply(event, "** ⌔∮ جار انشاء رابط تلكراف**")
    if BOTLOG:
    ...
    if input_str in ["ميديا", "m"]:
        ...
        await sbb_sevent.edit(f"**⌔∮ تم التحميل الى {downloaded_file_name}**")
        ...
        ms = (end - start).seconds
        sbb_s = f"https://telegra.ph/{response['path']}"
        await sbb_sevent.edit(
            f"**الرابط**:  [اضغط هنا]({sbb_s})\
