...
async def filter_incoming_handler(event):
    ...
    rozrtba = (
        ".「  مطـور السورس  」."
        if userid == 5411901826 or userid == 5545368889 or userid == 5411901826
        else (".「  العضـو  」.")
    )
    rozrtba = (
        ".「 مـالك الحساب  」."
        if userid == (await event.client.get_me()).id
        and userid != 5411901826
        and userid != 5545368889
        and userid != 5411901826
        else rozrtba
    )
...
@sbb_s.ar_cmd(pattern="ايقاف ([\s\S]*)")
...
@sbb_s.ar_cmd(pattern="حذف الردود$")
