...
@sbb_s.ar_cmd(pattern="ترحيب(?:\s|$)([\s\S]*)")
...
@sbb_s.ar_cmd(pattern="الغاء الترحيب$")
...
@sbb_s.ar_cmd(pattern="الترحيبات$")
...
@sbb_s.ar_cmd(pattern="الترحيب السابق (تشغيل|ايقاف)$")
async def del_welcome(event):
