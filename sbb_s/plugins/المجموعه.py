...
async def get_users(show):
    ...
    sbb_sevent = await edit_or_reply(show, "**⌔∮ جاري سحب قائمة معرّفات الأعضاء 🝛**")
    try:
        ...
        mentions += " " + str(e) + "\n"
    await edit_or_reply(sbb_sevent, mentions)


@sbb_s.ar_cmd(pattern="معلومات(?: |$)(.*)")
async def info(event):
    sbb_sevent = await edit_or_reply(event, "**⪼ يتمّ جلب معلومات الدردشة، إنتظر ⅏**")
    chat = await get_chatinfo(event, sbb_sevent)
    caption = await fetch_info(chat, event)
    try:
        await sbb_sevent.edit(caption, parse_mode="html")
    except Exception as e:
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, f"**⌔∮ هناك خطأ في معلومات الدردشة ✕ : **\n`{str(e)}`"
            )
        await sbb_sevent.edit("**⌔∮ حدث خطأ ما، يرجى التحقق من الأمر ⎌**")


async def get_chatinfo(event, sbb_sevent):
    ...
    try:
        ...
        try:
            ...
            await sbb_sevent.edit("**⌔∮ لم يتمّ العثور على القناة/المجموعة ✕**")
            return None
            ...
            await sbb_sevent.edit("**⌔∮ القناة أو المجموعة الخارقة غير موجودة ✕**")
            return None
        except (TypeError, ValueError) as err:
            await sbb_sevent.edit(str(err))
            return None
    return chat_info
