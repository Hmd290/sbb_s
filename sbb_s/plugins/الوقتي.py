...
from . import edit_delete, sbb_s, logging
...
autopic_path = os.path.join(os.getcwd(), "sbb_s", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "sbb_s", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "sbb_s", "photo_pfp.png")
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await sbb_s.upload_file(autophoto_path)
        try:
            if i > 0:
                await sbb_s(
                    functions.photos.DeletePhotosRequest(
                        await sbb_s.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await sbb_s(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await sbb_s(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await sbb_s(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
@sbb_s.ar_cmd(pattern="الصورة الوقتية$")
async def _(event):
    downloader = SmartDL(digitalpfp, digitalpic_path, progress_bar=False)
    downloader.start(blocking=False)
    while not downloader.isFinished():
...
@sbb_s.ar_cmd(pattern="بايو وقتي$")
async def _(event):
...
@sbb_s.ar_cmd(pattern="انهاء ([\s\S]*)")
async def _(event):
...
sbb_s.loop.create_task(digitalpicloop())
sbb_s.loop.create_task(autoname_loop())
sbb_s.loop.create_task(autobio_loop())
