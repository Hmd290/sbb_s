...
async def spam_function(event, redthon, sbb_s, sleeptimem, sleeptimet, DelaySpam=False):

    counter = int(sbb_s[0])
    if len(sbb_s) == 2:
        spam_message = str(sbb_s[1])
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            if event.reply_to_msg_id:
                await redthon.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and redthon.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            redthon = await event.client.send_file(
                event.chat_id, redthon, caption=redthon.text
            )
            await _sbb_sutils.unsavegif(event, redthon)
        ...
        if BOTLOG:
            ...
            redthon = await event.client.send_file(BOTLOG_CHATID, redthon)
            await _sbb_sutils.unsavegif(event, redthon)
        return
    elif event.reply_to_msg_id and redthon.text:
        spam_message = redthon.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
@sbb_s.ar_cmd(pattern="كرر (.*)")
async def spammer(event):
    redthon = await event.get_reply_message()
    ...
    await spam_function(event, redthon, sbb_s, sleeptimem, sleeptimet)


@sbb_s.ar_cmd(pattern=f"{TKRAR} (.*)")
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, sbb_s, sleeptimem, sleeptimet, DelaySpam=True)


@sbb_s.ar_cmd(pattern="تكرار الملصق$")
...
@sbb_s.ar_cmd(pattern="ايقاف التكرار ?(.*)")
