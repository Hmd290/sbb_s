...
@sbb_s.ar_cmd(pattern="تقليد$")
async def echo(event):
    if event.reply_to_msg_id is None:
        return await edit_or_reply(event, "⌔∮ يرجى الرد على الشخص الذي تريد ازعاجه ،")
    sbb_sevent = await edit_or_reply(event, "⌔∮ يتم تفعيل هذا الامر انتظر قليلا ،")
    user, rank = await get_user_from_event(event, sbb_sevent, nogroup=True)
    if not user:
        return
    ...
    try:
        ...
        await edit_delete(sbb_sevent, f"⌔∮ خطأ\n`{str(e)}`")
    else:
        await edit_or_reply(
            sbb_sevent,
...
@sbb_s.ar_cmd(pattern="الغاء تقليد$")
async def echo(event):
    if event.reply_to_msg_id is None:
...
@sbb_s.ar_cmd(pattern="حذف المقلدهم( للكل)?")
async def echo(event):
...
@sbb_s.ar_cmd(pattern="المقلدهم( للكل)?$")
async def echo(event):
    input_str = event.pattern_match.group(1)
    private_chats = ""
...
@sbb_s.ar_cmd(incoming=True, edited=False)
