...
async def admin_groups(sbb_s):
    sbb_sgroups = []
    async for dialog in sbb_s.iter_dialogs():
...
async def extract_time(sbb_s, time_val):
    if any(time_val.endswith(unit) for unit in ("s", "m", "h", "d", "w")):
        unit = time_val[-1]
        time_num = time_val[:-1]  # type: str
        if not time_num.isdigit():
            await sbb_s.edit("**- هذا الرقم غير صحيح  !**")
            return None
        if unit == "s":
