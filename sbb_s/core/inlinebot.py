...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**sbb_s"):
            buttons = [
                (
                    Button.inline("Stats", data="stats"),
                    Button.url("DEV", "https://t.me/redthon"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./sbb_s", "troll.txt")
            try:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./sbb_s", "secrets.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./sbb_s", "hide.txt")
            try:
            ...
            result = builder.article(
                title="sbb_s Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب ردثون **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/redthon"),
                Button.url(
                    "المطور",
                    "t.me/L3LL8",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل ردثون من نصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[𝙍𝙀𝘿𝙏𝙃𝙊𝙉 𝙐𝙎𝙀𝙍𝘽𝙊𝙏 🧸♥](https://t.me/redthon)",
...
@sbb_s.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@sbb_s.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@sbb_s.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@sbb_s.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
