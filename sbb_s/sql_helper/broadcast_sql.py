...
class redthonBroadcast(BASE):
    __tablename__ = "redthonbroadcast"
    ...
    def __repr__(self):
        return "<redthon Broadcast channels '%s' for %s>" % (
            self.group_id,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, redthonBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


redthonBroadcast.__table__.create(checkfirst=True)

redthonBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with redthonBROADCAST_INSERTION_LOCK:
        broadcast_group = redthonBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with redthonBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(redthonBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with redthonBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(redthonBroadcast.keywoard)
            .filter(redthonBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(redthonBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(redthonBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(redthonBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(redthonBroadcast).all()
        for x in all_groups:
