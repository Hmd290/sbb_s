...
class redthon_GlobalCollection(BASE):
    __tablename__ = "redthon_globalcollection"
    ...
    def __repr__(self):
        return "<redthon Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, redthon_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


redthon_GlobalCollection.__table__.create(checkfirst=True)

redthon_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with redthon_GLOBALCOLLECTION:
        keyword_items = redthon_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with redthon_GLOBALCOLLECTION:
        keyword_items = SESSION.query(redthon_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with redthon_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(redthon_GlobalCollection.keywoard)
            .filter(redthon_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(redthon_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(redthon_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(redthon_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(redthon_GlobalCollection).all()
        for x in all_groups:
